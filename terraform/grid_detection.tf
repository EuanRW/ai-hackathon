...
resource "aws_s3_object" "opencv_numpy_zip" {
  ...
  source = "${path.module}/utils/opencv-numpy.zip"
}
...
resource "aws_lambda_function" "grid_detection_function" {
  ...
  filename      = "${path.module}/utils/grid-detect-function.zip"
  ...
  source_code_hash = filebase64sha256("${path.module}/utils/grid-detect-function.zip")  # triggers updates
